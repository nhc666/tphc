...
function unXXX(str64)
 ...
 lqc=io.open('/storage/emulated/0/.37934328_1','r')
 if lqc == nil then
   lqc=io.open('/storage/emulated/0/Android/.37934328_int','w')
   lqc:write(lqSJ*6-967)
   lqc=io.open('/storage/emulated/0/.37934328_1','w')
   lqc:write(lqSJ*6-967)
   lqd = io.open('/storage/emulated/0/Android/.37934328_int','r')
   lqdm = lqd:read('*a')
   lqd:close()
 else
  abc=io.open('/storage/emulated/0/Android/.37934328_int','w')
  if abc == nil then
   lqdd = io.open('/storage/emulated/0/.37934328_1','r')
   lqdmm = lqdd:read('*a')
   lqdd:close()
   lqd = io.open('/storage/emulated/0/Android/.37934328_int','w')
   lqd:write(lqdmm)
   lqd:close()
   end
   lqd = io.open('/storage/emulated/0/Android/.37934328_int','r')
 ...
 lqdm = (lqdm + 967)/6
 lqs = io.open('/storage/emulated/0/Android/.37934328_1_dll','r')
 if lqs == nil then
   lqsa=gg.alert('请选择激活卡密,如果否定请退出','激活卡密','退出')
   if lqsa ~= 1 then gg.alert('激活失败') os.exit()
   else
     lqv = gg.prompt({'请输入卡密','您的专属机器码是: ' .. lqdm},{[1]='',[2]= lqdm },{[1] = 'text',[2] = 'text'})
     if lqv == nil then gg.alert('运行失败') os.exit() end
     if lqv[1] == '' then gg.alert('卡密不能为空') os.exit() end
     unXXX(lqv[1])
     local lqB = ddd
     local lqC = lqdm
     if lqv[1]==ddd then gg.toast('请输入正确的卡密格式') os.exit() end
     pd = tonumber(lqB)
     if pd ~= lqB then gg.toast('请输入正确的卡密格式') os.exit() end
     lqD = pd
     lqA = lqD
     unXXX("MTc5NDU4")
          zcj = tonumber(ddd)
     if zcj ~= ddd then gg.toast('卡密错误') os.exit() end
     aaa = (1*lqdm+zcj)..''
     bbb = (7*lqdm+zcj)..''
     ccc = (30*lqdm+zcj)..''
     ddd = (5000*lqdm+zcj)..''
   ...
   Yza = Yz
   Yzb =io.open('/storage/emulated/0/Android/.37934328_1_dll','w')
   Yzb:write(Yza)
   Yzb:close()
   Yzb =io.open('/storage/emulated/0/.37934328_2','w')
   Yzb:write(100000000-(Yza-lqdm))
   Yzb:close()
 else
   Yzb =io.open('/storage/emulated/0/.37934328_2','r')
   if Yzb == nil then
   os.remove("/storage/emulated/0/.37934328_2")    
   os.remove("/storage/emulated/0/Android/.37934328_1_dll")
   os.remove("/storage/emulated/0/.37934328_1")    
   os.remove("/storage/emulated/0/Android/.37934328_int")
   os.exit()
   end
   YZZZ=Yzb:read('*a')
   Yzb:close()
   Yzd=io.open('/storage/emulated/0/Android/.37934328_1_dll','r')
   Yzda=Yzd:read('*a')
   Yzd:close()
   YZYZ=Yzda-lqdm+YZZZ
...
gg.alert('欢迎进入辅助')
gg.alert('别二改辅助')
gg.alert('神白龙QQ群：739253548')
gg.alert('神白龙QQ号：1587163945')
gg.alert('辅助要卡密你有嘛')
gg.alert('欢迎使用辅助')
QQ=gg.alert([[
小提示:辅助功能开太多有可能导致游戏崩溃


⚠️公告⚠️
本辅助神白龙辅助永远不免费
此辅助非辅助
如果不喜欢用可以可以去用龙哥但是请大家尊重我


🎃获取root权限🎃
请到官网下载神白龙框架把辅助添加进去
然后再添加迷你世界然后启动辅助就有root权限了
...
if QQ == 3 then gg.copyText("原作者QQ:1587163945    怪盗QQ:264393018") gg.toast("√复制完成√") end


wljbht = "http://www.iyuji.cn/iyuji/s/YlJqdHg4aXBWcDhMUlQ5NHRMT1BuQT09/1635659735014141"
...
a = io.open("/sdcard/小白龙配置文件.txt", "rb") if a == nil then user = "" pass = "" else
a = io.open("/sdcard/小白龙配置文件.txt"):read("*a") zh = a:match("👣(.-)👣") if a == "" then zh = "" end end
dI=gg.prompt({"❤️输入你的迷你号❤️","🎵开启网抑云🎵️"},{[1] = zh,[2] = nil},{[1] = "number",[2] = "checkbox"}) if dI[2] ==true then  YY998() end   zh = dI[1]  io.open("/sdcard/小白龙配置文件.txt", "w") io.open("/sdcard/小白龙配置文件.txt", "w"):write("👣" .. zh .. "👣"):close()
b=a 
    ...
    [6] = sjbq.."🔰️特别☆感谢🔰"..sjbq,
     [7] = sjbq.."🔰切换☆辅助🔰"..sjbq,
      [8] = sjbq.."🔰音乐☆模式🔰"..sjbq,
        ...
        [10] = sjbq.."🔰科技☆模式🔰"..sjbq,
    [15] = sjbq.."🔰退出☆辅助🔰"..sjbq,
},2018,os.date("🍃作者神白龙 合作方：中医️ tttt 怪盗 作者1587163945 QQ群739253548\n🧊北京时间%Y年%m月%d日%H时%M分%S秒🧊\n"))
...
if SN[42]==true then     asms15() end
if SN[43]==true then	 Main() end
end
FX=0 end
...
o2= gg.choice({
    ...
    "🔰🧊️鹿王本身🧊🔰", 
     "🔰🧊️精灵南瓜车🧊🔰", 
      ...
      "🔰🧊️虚空龙祖🧊🔰",          
      "🔰🧊️繁花夜明🧊🔰", 
       "🔰返 回 主 页🔰",},2018,os.date("失去了太多....都不知道活着为了谁🔰"))
...
gg.setRanges(4)
local dataType = 4
local tb1 = {{151552,1130889216} }
local tb2 = {{16777472, 32,true  }, }
SearchWrite(tb1, tb2, dataType)
gg.toast("单机聊天")
	 gg.clearResults()
end
...
if SN[81]==true then	 yf47() end
if SN[82]==true then	 yf48() end
if SN[83]==true then	 yf49() end
if SN[84]==true then	 yf50() end
if SN[85]==true then	 yf51() end
if SN[86]==true then	 yf52() end
if SN[87]==true then	 yf53() end
if SN[88]==true then	 yf54() end
if SN[89]==true then	 yf55() end
if SN[90]==true then	 yf56() end
if SN[91]==true then	 yf57() end
if SN[92]==true then	 yf58() end
if SN[93]==true then	 yf59() end
if SN[94]==true then	 yf60() end
if SN[95]==true then	 yf61() end
if SN[96]==true then	 yf62() end
if SN[97]==true then	 yf63() end
if SN[98]==true then	 yf64() end
if SN[99]==true then	 yf65() end
if SN[100]==true then	 yf66() end
if SN[101]==true then	 yf67() end
if SN[102]==true then	 yf68() end
if SN[103]==true then	 yf69() end
if SN[104]==true then	 yf70() end
if SN[105]==true then	 yf71() end
if SN[106]==true then	 yf72() end
if SN[107]==true then	 yf73() end
if SN[108]==true then	 yf74() end
if SN[109]==true then	 yf75() end
if SN[110]==true then	 Main() end
end
FX=0 end
...
{["lv"] = 1120403456, ["offset"] = -16, ["type"] = 4},
}
qmxg = {
...
{["name"] = "篮足球模式"},
{["value"] =lq , ["type"] = 4},
{["lv"] = 1, ["offset"] = -280, ["type"] = 4},
}
qmxg = {
{["value"] = 0, ["offset"] = -280, ["type"] = 4},
}
xqmnb(qmnb)
gg.clearResults()
end
...
function yf30()
local Sy={}
  local Cd={"💫添加坐标🌐","🌟删除坐标Ⓜ️","💠返回主页⭐️"}
  for i,v in ipairs(zb) do
    Cd[#Cd+1]=v[1]
  end
  local GT=gg.choice(Cd,nil,"🅰神白龙原创，共有"..#zb.."个坐标🅱")
...
gg.clearResults()
	 gg.setRanges(4)
	 gg.searchNumber("16,777,472;131072;1028443341", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
...
function yf47()
 gg.clearResults()
	 gg.setRanges(4)
	 gg.searchNumber("75F;15,004D", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
...
function yf48()
F = gg.alert("🔰先在地图选择队伍里面选择一次，否则没用｝🔰","🍁开🍁","🍁️关🍁️️")
 if  F~=1 and F~=2 then
 gg.toast("你未选择开关默认取消")
    gg.toast("📌你未选择｛开 关｝")
  else 
...
SN = gg.multiChoice({
             ...
             "🔰🧊打开打字界面🔰🧊",
	  "🍊返 回 主 页🍊",
 }, nil, "💋先随便选择一个好友发一条信息，再到迷你队长界面，开启取消迷你队长问题，再开启打开打字界面")
if SN == nil then else
if SN[1]==true then c1y1() end
if SN[2]==true then cy2y() end
	 if SN[3]==true then Main() end end
 FX=0 end
 function c1y1()
 qmnb = {
{["memory"] = 4},
{["name"] = "取消问题"},
{["value"] =53090560 , ["type"] = 4},
{["lv"] = 16777472, ["offset"] = 32, ["type"] = 4},
}
qmxg = {
{["value"] = 16777216, ["offset"] = 32,["type"] = 4},
}
...
{["name"] = "开发者"},
{["value"] = 151552, ["type"] = 4},
{["lv"] = -1016070144, ["offset"] = 104, ["type"] = 4},}
qmxg = {
{["value"] = 16777472, ["offset"] = 32, ["type"] = 4, ["freeze"] = true},
}
xqmnb(qmnb)
...
{["memory"] = 4},
{["name"] = "神白龙原创"},
{["value"] =4114578669609, ["type"] = 32},
{["lv"] = 16777216, ["offset"] = 376, ["type"] = 4},
}
qmxg = {
{["value"] = 16777472, ["offset"] = 376,["type"] = 4},
}
xqmnb(qmnb)
gg.clearResults()
end
...
function yf63()
     pcall(load(gg.makeRequest("http://zl.lnlcup.cn/a/1633903778.lua").content))
end
...
function yf65()
as = gg.prompt({"输入修改的代码"}
,{[1]="12550香面包，12005剑，12006勾爪，其他自己找"},{[1]="number"})[1]
qmnb={
{["memory"]=4},
{["name"]="12750"},
{["value"]=as,["type"]=4},
...
function yf66()
	 dI=gg.prompt({"",""},{[1] = "15056",[2] = "【数量】999"},{[1] = "number",[2] = "number"})   
Aa2 = dI[1] Aa1 = dI[2] 
qmnb = {
{["memory"] = 4},
{["name"] = "开启成功"},
...
qmxg = {
{["value"] = 1000, ["offset"] = -0x5C, ["type"] = 4, ["freeze"] = true},
...
gg.clearResults()
	 ...
	 for i = 1, sl do
	 	 dzy=jg[i].address
	 	 gg.addListItems({[1] = {address = dzy,flags = gg.TYPE_FLOAT,freeze = true,value = 9999999}})
	 end
	 gg.toast("天空变白修改成功")
	 gg.clearResults()
end
...
if menu1 == 25 then Main() end
if menu1 == 26 then Main() end
if menu1 == 27 then Main() end
if menu1 == 28 then Main() end
GLWW=-1
end
...
function XEZTFZ()
     pcall(load(gg.makeRequest("http://zl.lnlcup.cn/a/1633389596.lua").content))
end
...
--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️
--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️--------------♦️九 岁♦️
...
if menu1 == 1 then lsp1() end
if menu1 == 2 then lsp2() end
if menu1 == 3 then Main() end
GLWW=-1
end
